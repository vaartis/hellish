...
package body Hellish_Web.Routes is
   ...
   begin
      ...
      declare
         ...
         Found_Torrents : Torrent_List := Database.Search_Torrents(Query, Page_Offset, Page_Size);
         Total_Count : Natural := Database.Search_Count(Query);
         -- Round up
         Page_Count : Natural := Natural(Float'Ceiling(Float(Total_Count) / Float(Page_Size)));

         Torrent_Names, Torrent_Ids,
           Torrent_Uploaders : Vector_Tag;
         Pages : Vector_Tag;
      begin
         ...
         if Page_Count > 1 then
            if Page_Count > 10 then
               ...
               Insert(Translations, Assoc("last_page", Page_Count));
            else
