...
package body Hellish_Web.Database is
   ...
   procedure Migrate (Session : Session_Type) is
      ...
      Database_Exists_Query : Prepared_Statement :=
        Prepare
          ("SELECT EXISTS(SELECT * FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'config');");
   begin
      ...
      while Current_Version < Latest_Version loop
         ...
         Start_Search
           (Search    => Dir_Search,
            ...
            Pattern   => Trim (Current_Version'Image, Ada.Strings.Left) & "_*.sql");
   ...
   function Create_User (Name, Password : String; Created_User : out Detached_User'Class) return Boolean is
      ...
      Password_Hash : Any_Hash := Generate_Password_Hash (Criticality => Online_Interactive, Password => Password);
   ...
   begin
      return
        Detached_User (User_Data) /= No_Detached_User and then Password_Hash_Matches (User_Data.Password, Password);
   ...
   procedure Notify_User (The_User : Detached_User'Class; Notification : String) is
      ...
      Sub_Notifications : Json_Array :=
        (if Has_Field (Sub_Profile, "notifications") then Get (Sub_Profile, "notifications") else Empty_Array);
   begin
      ...
      if Sub_Profile.Has_Field ("email") then
         ...
         begin
            if Profile_Email.Has_Field ("address")
              and Profile_Email.Has_Field ("notifications")
              and Boolean'(Profile_Email.Get ("notifications"))
            then
               ...
               begin
                  Hellish_Mail.Send
                    (Aws.Smtp.E_Mail (The_User.Username, Address),
      ...
      Session.Db.Execute
        (Sql_Update
           (Table => Torrents,
            ...
            Set   => (Torrents.Group = (if Group = -1 then Null_Field_Integer else Expression (Group)))));
      Session.Commit;
   end Set_Torrent_Group;

   Update_User_Torrent_Stats_Stmt : Prepared_Statement :=
     Prepare
       ("UPDATE user_torrent_stats SET "
        & "uploaded=uploaded + $3, downloaded=downloaded + $4 WHERE by_user = $1 AND of_torrent = $2;",
        On_Server => True);
   Update_User_Stats              : Prepared_Statement :=
     Prepare
       ("UPDATE users SET " & "uploaded=uploaded + $2, downloaded=downloaded + $3 WHERE id = $1;", On_Server => True);
   procedure Update_Torrent_Up_Down
     (User            : Detached_User'Class;
      Info_Hash       : String;
      Uploaded_Diff   : Long_Long_Integer;
      Downloaded_Diff : Long_Long_Integer)
   is
      Session : Session_Type := Get_New_Session;

      T_List : Torrent_List := All_Torrents.Filter (Info_Hash => Info_Hash).Get (Session);
      S_List : User_Torrent_Stat_List :=
        All_User_Torrent_Stats.Filter (By_User => User.Id, Of_Torrent => T_List.Element.Id).Get (Session);

      Stats : Detached_User_Torrent_Stat'Class :=
        (if S_List.Has_Row then S_List.Element.Detach else New_User_Torrent_Stat);
   begin
      if not S_List.Has_Row then
         ...
         Execute
           (Session.Db,
            Update_User_Torrent_Stats_Stmt,
            Params => [+User.Id, +T_List.Element.Id, As_Bigint (Uploaded_Diff), As_Bigint (Downloaded_Diff)]);
      end if;

      -- Also add it to the total user credit
      Execute
        (Session.Db, Update_User_Stats, Params => [+User.Id, As_Bigint (Uploaded_Diff), As_Bigint (Downloaded_Diff)]);

      Session.Commit;
   end Update_Torrent_Up_Down;

   function Get_Torrent_By_Hash
     (Info_Hash : String; Session : Session_Type := Get_New_Session) return Detached_Torrent'Class
   is
   ...
   function Torrent_Snatches (The_Torrent : Detached_Torrent'Class) return Integer is
      ...
      Snatches_Query : Sql_Query :=
        Sql_Select
          (From   => User_Torrent_Stats,
           Fields => Apply (Func_Count, User_Torrent_Stats.Snatched),
           Where  => (User_Torrent_Stats.Of_Torrent = The_Torrent.Id) and (User_Torrent_Stats.Snatched = True));
   begin
   ...
   is
      ...
      -- Only search by display_name if query is not empty
        ((Text_Param (1) = "") or Ilike (Torrents.Display_Name, Concat ("%" & Text_Param (1) & "%")))
        and
        -- Only search by uploader if not 0
        ((Integer_Param (2) = 0) or Torrents.Created_By = Integer_Param (2))
        and
        -- Only search by category if it's not -1
        ((Integer_Param (3) = -1) or Torrents.Category = Integer_Param (3))
        and
        -- Only search by snatcher if it's not -1
        ((Integer_Param (4) = -1)
         or (User_Torrent_Stats.By_User = Integer_Param (4) and User_Torrent_Stats.Snatched = True));
   ...
   procedure Delete_Torrent (Id : Integer) is
      ...
      Stats_Delete    : Sql_Query :=
        Sql_Delete (From => User_Torrent_Stats, Where => User_Torrent_Stats.Of_Torrent = Id);
      Torrent_Delete  : Sql_Query := Sql_Delete (From => Torrents, Where => Torrents.Id = Id);
      Comments_Delete : Sql_Query := Sql_Delete (From => Posts, Where => Posts.Parent_Torrent = Id);
   ...
   function Torrent_Comments
     (Parent_Torrent : Integer;
   ...
   is
      ...
      Session           : Session_Type := Get_New_Session;
      The_Post_Managers : Posts_Managers :=
        All_Posts.Order_By (Asc (Posts.Id)).Filter (Parent_Torrent => Parent_Torrent);

      Count_Cur   : Direct_Cursor;
      Count_Query : Sql_Query :=
        Sql_Select
          (From => Posts, Fields => Apply (Func_Count, Posts.Id), Where => (Posts.Parent_Torrent = Parent_Torrent));
   begin
   ...
   function Get_User_Stats_For_Torrent
     (User : Detached_User'Class; Torrent : Integer) return Detached_User_Torrent_Stat'Class
   is
      Session    : Session_Type := Get_New_Session;
      Stats_List : User_Torrent_Stat_List :=
        All_User_Torrent_Stats.Filter (By_User => User.Id, Of_Torrent => Torrent).Get (Session);
   begin
   ...
   is (Get_Torrent_Group (Get_New_Session, Id));
   ...
   begin
      return (if Group_List.Has_Row then Group_List.Element.Detach else No_Detached_Torrent_Group);
   end Get_Group;
   ...
   function Get_Group_Torrents (Id : Integer) return Direct_Torrent_List is
      ...
      Session : Session_Type := Get_New_Session;
   begin
      return All_Torrents.Filter (Group => Id).Order_By (Desc (Torrents.Id)).Get_Direct (Session);
   end;

   function Search_Torrent_Groups
     (Query       : String;
   ...
   is
      ...
      -- Only search by name if query is not empty
        ((Text_Param (1) = "") or Ilike (Torrent_Groups.Name, Concat ("%" & Text_Param (1) & "%")));

      Session            : Session_Type := Get_New_Session;
      The_Query_Managers : Torrent_Groups_Managers :=
        All_Torrent_Groups.Limit (Limit, From => Offset).Order_By (Desc (Torrent_Groups.Id)).Filter (Search_Criteria);
   ...
   is (Orm.Get_Post (Get_New_Session, Id));
   ...
   is
      ...
      Session           : Session_Type := Get_New_Session;
      The_Post_Managers : Posts_Managers := All_Posts.Order_By (Asc (Posts.Id)).Filter (Parent_Post => Parent_Post);
   ...
   function Get_Latest_News return Detached_Post'Class is
      ...
      News_List : Post_List := All_Posts.Filter (Flag => 1).Limit (1).Order_By (Desc (Posts.Id)).Get (Session);
   begin
   ...
   is
      ...
      Search_Criteria : Sql_Criteria :=
      -- Only search by display_name if query is not empty
        ((Text_Param (1) = "") or Ilike (Hellish_Database.Posts.Title, Concat ("%" & Text_Param (1) & "%")))
        and
        -- Only show posts without parents
        (Is_Null (Posts.Parent_Post) and Is_Null (Posts.Parent_Torrent))
        and ((Integer_Param (2) = -1) or Hellish_Database.Posts.Flag = Integer_Param (2))
        and
        -- Only search by author if name is not zero
        ((Integer_Param (3) = 0) or Hellish_Database.Posts.By_User = Integer_Param (3));

      Session            : Session_Type := Get_New_Session;
      The_Query_Managers : Posts_Managers :=
        All_Posts.Limit (Limit, From => Offset).Order_By (Desc (Posts.Id)).Filter (Search_Criteria);
   ...
   Peers_Insert_Statement : Prepared_Statement :=
     Prepare
       ("INSERT INTO peer_data (torrent_id, data) VALUES ($1, $2) "
   ...
   function Add_Uploaded_Image (Username : String; Filename : String) return Detached_Image_Upload'Class is
      ...
      Image_List : Image_Upload_List :=
        All_Image_Uploads.Filter (By_User => The_User.Id, Filename => Filename).Get (Session);
   ...
   begin
      ...
      return All_Image_Uploads.Filter (Filename => The_Image.Filename).Get (Session).Has_row;
   ...
   begin
      return
        All_Invites.Filter (Activated => True).Limit (Limit).Select_Related (Depth => 1).Order_By
          (Desc (Hellish_Database.Invites.For_User))
          .Get (Session);
   end;

   Channel_Insert_Statement : Prepared_Statement :=
     Prepare
       ("INSERT INTO irc_channels (name, data) VALUES ($1, $2) "
   ...
   is (All_Irc_Channels.Get (Get_New_Session));
   ...
   package body Subscriptions is
      procedure Subscribe (User : Detached_User'Class; To : T) is
         ...
         Subscriptions    : Json_Array :=
           (if Has_Field (The_Meta, "subscribed") then Get (The_Meta, "subscribed") else Empty_Array);
         Unsubscribed     : Json_Array :=
           (if Has_Field (The_Meta, "unsubscribed") then Get (The_Meta, "unsubscribed") else Empty_Array);
